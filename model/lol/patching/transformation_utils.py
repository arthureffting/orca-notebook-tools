...
def xyxy_to_points(xyxy, scale=1.0):
    points = []
    for batch in xyxy:
        for prediction in batch:
            confidence = prediction[0].item()
            upper_point = Point(prediction[1].item() / scale, prediction[2].item() / scale)
            lower_point = Point(prediction[3].item() / scale, prediction[4].item() / scale)
            points.append((upper_point, lower_point, confidence))
    return points
