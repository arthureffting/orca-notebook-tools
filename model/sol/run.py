...
## Runs the model ona  given image and returns only the predictions with a confidence above a certain threshold
def run_sol(model_path, img_path, confidence_threshold, rescale_range=[640, 640]):
    '''
    ...
    for batch in predictions:
        for prediction in batch:
            if prediction[0].item() >= confidence_threshold:
                results.append(prediction[1:5])
    return results


results = run_sol("snapshots/sol/training/sol.pt", "test_image.jpg", confidence_threshold=0.1)
print(len(results), "results accepted")
